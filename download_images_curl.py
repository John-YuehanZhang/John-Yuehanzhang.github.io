...
def download_image_curl(url, save_path):
    ...
    try:
        print(f"  下载: {url}")
        cmd = ['curl', '-k', '-L', '-o', save_path, url]
        result = subprocess.run(cmd, capture_output=True, timeout=30)
        if result.returncode == 0 and os.path.exists(save_path):
            size = os.path.getsize(save_path)
            print(f"  ✓ 保存到: {save_path} ({size} 字节)")
            return True
        else:
            print(f"  ✗ 错误: curl失败，返回码 {result.returncode}")
...
updated_outline = update_html_file(outline_file, downloaded_mapping)
updated_dqi = update_html_file(dqi_file, downloaded_mapping)
